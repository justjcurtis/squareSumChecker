...
process.on('message', function(message) {
    ...
    function handleMessage(message) {
        if (message.squares && message.currentMax) {
            const { squares, currentMax, prevMap } = message
            try {
                const g = new Graph(squares, 1, currentMax, prevMap);
                const path = findRoute(g)
                process.send({ path, currentMax, map: g.squareSumsMap })
            } catch (err) {
                console.log(err)
            }
        }
        if (message.kill) process.disconnect()
    }
});
